...
export default function ImageDetail({
  ...
  return (
    // On large screens the panel is capped to the viewport height (leaving
    // room for the sticky offset and the "Close detail" button below), so its
    // body scrolls internally while the result list behind it stays put.
    <section
      ref={bodyScrollRef}
      className="flex flex-col overflow-hidden rounded-xl border border-neutral-200 bg-white p-5 shadow-sm lg:max-h-[calc(100vh-7rem)] overflow-y-auto overscroll-contain"
    >
      {/* Pinned region: title and preview always stay visible at the top. */}
      ...
      {/* Independently scrollable body; overscroll is contained so the
          result list / page behind it never scrolls along. */}
      <div className="min-h-0 flex-1">
            ...
            <div>
              ...
              <dd className="mt-1 space-y-1">
                {result.occurrences.map((occurrence) => (
                  <p
                    key={`${occurrence.pageUrl}-${occurrence.imageUrl}`}
                    className="break-all text-neutral-700"
                  >
                    {occurrence.pageUrl}
                  </p>
                ))}
              ...
              return (
                ...
                >
                  ...
                  <p className="mt-1 text-xs text-neutral-500">
                    via {providerLabel(match.providerId)}
                  </p>
                </li>
              );
            })}
              ...
              {result.providerOutcomes.map((outcome) => {
                ...
                const badge = outcome.searched
                  ? outcome.status === "MATCH_FOUND"
                    ? {
                        text: `${outcome.matchCount} match${outcome.matchCount === 1 ? "" : "es"}`,
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span
                        className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${badge.classes}`}
                      >
                        {badge.text}
                      </span>
                    </div>
        ...
        {onRetry &&
        (result.status === "FAILED" || hasFailedProviderLookup(result)) ? (
          <button
            type="button"
            onClick={async () => {
              setRetrying(true);
        ...
        <p className="mt-6 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs leading-relaxed text-amber-800">
          Screening hint only: a match means the image exists elsewhere. It does
          not establish copyright ownership or infringement. Verify the source
